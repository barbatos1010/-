  ...
  <div class="container">
    ...
    <div class="make">
      ...
      >
        ...
        <el-form-item label="作者" size="mini" prop="curreatTeacherId">
          <el-select size="mini" v-model="paperFrom.curreatTeacherId" placeholder="请选择作者">
            <el-option
      ...
      <el-table :data="testPaperList" style="100%">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini" @click="editTestPaper(scope.row)">编辑</el-button>
            <el-button size="mini" type="danger" @click="deleteTestPaper(scope.row.tpId)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页器 -->
...
export default {
  ...
  computed: {
    ...mapState(["teachers", "coursess", "testPaperPages", "testPaperList"])
  },
  methods: {
    ...mapActions(["getTeachers", "getCoursess", "getTestPaperList"]),
    //重置表单
    resetForm(formName) {
    ...
    async makePaper() {
      ...
      let data = await this.$api.API_MAKE_PAPER(
        ...
        `?uid=${formData.curreatTeacherId}`
      );
      if (data.code == 1) {
        let teachers = this.teachers;
        let index = teachers.findIndex(el => {
          return el.userUid === formData.curreatTeacherId;
        });
        this.$router.push({
          name: "editTestPaper",
          query: {
            ...
            author: teachers[index].userName
          }
        });
      }
    },
